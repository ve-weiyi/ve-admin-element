...
import type { TalkNewReq } from "@/api/types";
import { ElMessage } from "element-plus";
import { TalkStatusEnum, TalkTopEnum } from "@/enums/TalkEnum";
...
function findTalk(talkId: number) {
  if (talkId) {
    TalkAPI.getTalkApi({ id: talkId }).then((res) => {
...
function saveOrUpdateTalk() {
  ...
  if (talk.value.id) {
    TalkAPI.updateTalkApi(talk.value).then((res) => {
    ...
    TalkAPI.addTalkApi(talk.value).then((res) => {
      editorRef.value.clear();
      uploadList.value = [];
