...
export function uploadFile(blob: Blob, path: string) {
  if (path.startsWith("/")) {
    ...
    return Promise.reject(new Error("上传路径不能以'/'开头"));
  }

  if (!path.endsWith("/")) {
    ElMessage.error("上传路径必须以'/'结尾");
    return Promise.reject(new Error("上传路径必须以'/'结尾"));
  }
  ...
  return UploadAPI.uploadFile(data);
}
...
export async function multipleUploadFile(files: Blob[], path: string) {
  ...
  for (const file of files) {
    ...
    results.push(await UploadAPI.uploadFile(data));
  }
  return results;
}
