  ...
  <div class="app-container">
    ...
    >
      ...
      <template #icon="scope">
        <img
          ...
          class="article-cover"
          style="cursor: pointer"
          @click="handleOpen(scope.row as FileInfoVO)"
        />
    ...
    >
      ...
      <el-scrollbar max-height="60vh">
        ...
        >
          <el-form-item label="文件路径" prop="file_base">
            <el-input v-model="importFormData.file_base" />
          </el-form-item>
...
const { contentRef, handleSearchClick, handleFilterChange } = usePage();

// 其他工具栏
function handleToolbarClick(name: string) {
  switch (name) {
    case "upload":
      importFormData.file_base = normPath(currentPath.value);
...
async function handleImport() {
  multipleUploadFile(
    importFormData.files.map((file) => {
      return file.raw as Blob;
    }),
    importFormData.file_base
  ).then(() => {
    fetchPageData();
...
function fetchPageData() {
  const queryParams = {
    file_base: currentPath.value,
  };
  contentRef.value?.fetchPageData(queryParams);
}

const handleOpen = (row: FileInfoVO) => {
  currentPath.value = normPath(row.file_base);
  fetchPageData();
};

const handleBreadcrumbClick = (path: string) => {
  currentPath.value = normPath(path);
  fetchPageData();
};
</script>
<style scoped>
