...
function update() {
  nextTick(() => {
    ...
    for (let j = 0; j < config.mcList.length; j++) {
      const rx1 = config.mcList[j].cx;
      const ry1 = config.mcList[j].cy * config.ca + config.mcList[j].cz * -config.sa;
      const rz1 = config.mcList[j].cy * config.sa + config.mcList[j].cz * config.ca;
